...
public class PushMsgReceiver extends PushMessageReceiver{
    ...
    public void onReceivePassThroughMessage(Context context, MiPushMessage miPushMessage) {//透传消息到达
        ...
        if(!TextUtils.isEmpty(content)){
            ...
            if(PushMessageType.AUTH_LOGIN.equals(bizType)){//授权登录消息

                LogUtils.i(TAG,"授权登录消息");

            }else if(PushMessageType.VERIFY_COUPON_REMIND.equals(bizType)){//验券提醒消息

                LogUtils.i(TAG,"验券提醒消息");

            }else if(PushMessageType.CANCEL_AUTH_LOGIN.equals(bizType)){//老板端取消授权

                LogUtils.i(TAG,"老板端取消授权");

            }else if(PushMessageType.CHANGE_AUTH_RIGHT.equals(bizType)){//修改员工设备权限、设备名

                LogUtils.i(TAG,"修改员工设备权限、设备名");

            }else if(PushMessageType.CHANGE_TABLE_SWITCH.equals(bizType)){//老板设置桌位开关

                LogUtils.i(TAG,"老板设置桌位开关");

            }else if(PushMessageType.ORDER_MESSAGE.equals(bizType)){//用户到店，订单 —— 用户扫码入座

                LogUtils.i(TAG,"用户到店，订单 —— 用户扫码入座");
                doWhat(context,true, "有新订单", "您有未处理的新订单！",msg,miPushMessage.getNotifyType(),mainActivity);

            }else if(PushMessageType.ADD_ORDER_MESSAGE.equals(bizType)){//加菜订单

                LogUtils.i(TAG,"加菜订单");
                doWhat(context,true,"顾客加菜",msg.getDialogContent(),msg,miPushMessage.getNotifyType(),mainActivity);

            }else if(PushMessageType.USER_SEAT_MSG.equals(bizType)){//用户点击我已入座

                LogUtils.i(TAG,"用户点击我已入座");
                doWhat(context, true, "有新订单", "您有未处理的新订单！", msg, miPushMessage.getNotifyType(), mainActivity);

            }else if(PushMessageType.USER_PAY_SUCCESS_MSG.equals(bizType)){//用户付款成功

                LogUtils.i(TAG,"用户付款成功");
                doWhat(context,true,"收到餐费",msg.getDialogContent(),msg,miPushMessage.getNotifyType(),mainActivity);

            }else if(PushMessageType.USER_CANCEL_MAG.equals(bizType)){//用户取消订单

                LogUtils.i(TAG,"用户取消订单");

            }else if(PushMessageType.CHANGE_TEXT_FONT.equals(bizType)){//（修改）接收后到修改打印设置内容，包括字体 联数 顾客下单自动打印厨房单 小票

                LogUtils.i(TAG,"（修改）接收后到修改打印设置内容，包括字体 联数 顾客下单自动打印厨房单 小票");

            }else if(PushMessageType.MEMBER_TO_CHANGE_PRICE.equals(bizType)){//会员改价

                LogUtils.i(TAG,"会员改价");

            }else if(PushMessageType.MEMBER_TO_CANCEL_CHANGE_PRICE.equals(bizType)){//取消会员改价

                LogUtils.i(TAG,"取消会员改价");

            }else if(PushMessageType.PRINT_FROM_THIS_DEVICE.equals(bizType)){//其他设备用过本机打印

                LogUtils.i(TAG,"其他设备用过本机打印");

            }else if(PushMessageType.PRINT_DEVICE_CONNECT_STATE.equals(bizType)){//已选择打印机状态改变
